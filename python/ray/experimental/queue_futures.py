...
class Queue(object):
    ...
    def get(self):
        oid_bytes = np.random.bytes(20)
        self.actor.get.remote(oid_bytes)
        return ray.get(ray.ObjectID(oid_bytes))
...
class _QueueActor(object):
    ...
    def put(self, item):
        if self.maxsize > 0 and self._qsize() >= self.maxsize:
            return False
        if len(self.waiting):
            oid = self.waiting.popleft()
            ray.worker.global_worker.store_and_register(oid, item)
        else:
            self._put(item)
        return True

    def get(self, oid_bytes):
        oid = ray.ObjectID(oid_bytes)
        if self._qsize():
            item = self._get()
            ray.worker.global_worker.store_and_register(oid, item)
        else:
            self.waiting.append(ray.ObjectID(oid_bytes))
